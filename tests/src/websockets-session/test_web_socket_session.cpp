...
namespace {
  ...
  void _runServer(
    ...
    _webSocketServer->setOnMessageCallback(
      [useStrands, &_serverMutex, &serverReceived](std::shared_ptr<IWebSocketSession> wsSession, const char* dataPtr, std::size_t dataLength) {
    ...
    _webSocket->setOnMessageCallback([useStrands, &_clientMutex, &clientReceived](const uint32_t sizeReceived, const uint8_t* dataReceived) {
      // std::cout << "[CLIENT] new message -> " << std::string_view(reinterpret_cast<const char*>(dataReceived), sizeReceived) << std::endl;
    ...
    for (int32_t ii = 0; ii < 10; ++ii) {
      ...
      if (
        serverReceived.size() == totalMsg &&
        clientReceived.size() == totalMsg
      ) {
        break;
      }
